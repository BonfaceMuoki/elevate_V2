...
import {
    useGetSupplierProductsQuery, useUpdateOrderProductStatausMutation,
    ...
    return (
        <ul className="dtr-details p-2 border-bottom ms-1">
            <li className="d-block d-sm-none mb-10">
                <span className="dtr-title">Line Total</span> <span className="dtr-data"><LineTotal order={data} /></span>
            </li>
            <li className="d-block d-sm-none  mb-10">
                <span className="dtr-title ">Order Status</span> <span className="dtr-data"> <OrderStatus order={data} /></span>
            </li>
            <li>
                <span className="dtr-title">Remove From Cart</span> <span className="dtr-data"> <ProductsList order={data} /></span>
            </li>
        </ul>
    );
};
const NoDataInCard = () => {
    ...
    return (
        <div style={{ width: '100%' }}>
            <Alert className="alert-icon" color="primary" style={{ margin: "50px", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" }} >
                <div>
                    <p> <Icon name="alert-circle" /> You do not have orders.</p>
                </div>
                <div>
    ...
    return (
        <div className="user-card">
            <UserAvatar
                size="sm"
                theme="primary"
                text={findUpper(row?.owner?.full_name)}
                image={""}
            ></UserAvatar>
...
const ProductsList = ({ order, intiateRefresh }) => {
    ...
    return (
        <>
            ...
            <Modal size="md" isOpen={modalShowoProductManage} toggle={togglemodalShowoProductManage}>
                ...
                <ModalBody >
                    <form onSubmit={handleUpdateProductStatus(submitProductStatus)}  >
                        <Row >
                            <Col md="6">
                                <div className="form-group">
                                    ...
                                    <select className="form-control" defaultValue={activeOrderProduct?.delivery_status} {...updatestatusform('product_delivery_status')} >
                            ...
                            <Col md="6" >
                                <div className="form-group">
                                    ...
                                    <select className="form-control" defaultValue={activeOrderProduct?.payment_status}  {...updatestatusform('product_payment_status')}>
                        ...
                        <Row>
                            <Col className="mt-1" md="12" style={{ display: "flex", flexDirection: "row", justifyContent: "flex-end" }} ><Button className="btn-md" color="primary">Update</Button></Col>
                        </Row>
                                    ...
                                    order.order_products.map((prod, index) => {
                                        ...
                                        return (<tr>
                                            <td>
                                                ...
                                                {prod?.product?.supplier?.supplier_name}<br></br>
                                                <span className="text-muted" ><strong>Phone :</strong><br></br>
                                                    {prod?.product?.supplier?.supplier_phone}</span>
                                            </td>
                                            <td>{prod?.product?.product_name}</td>
                                            <td>{formatNumberAddCommas(prod?.total_product_cost / prod?.quantity_bought)}</td>
                                            <td>{prod?.quantity_bought}</td><td>{formatNumberAddCommas(prod?.total_product_cost)}</td>
                                            <td>
                                            ...
                                            <td>
                                                <Button color="info" onClick={() => showOrderProductsManagement(prod)} >Manage</Button>
                                            </td>
                                        </tr>);
                                    }) : <div>No Products</div>
                            }
            ...
            {/* ///modal add supplier */}
            < Button color="primary" onClick={() => showOrderProductsModal(order)} style={{ marginRight: "10px" }} > <Icon name="eye" >  </Icon>  Products</Button >
...
const LineTotal = ({ order }) => {
    ...
    return (<div className="cart-action-container" >
        {
            (order?.total_order_cost != null) &&
            formatNumberAddCommas(order?.total_order_cost)
        }
    </div>);
};
function AdminOrders() {
    ...
    const { data: orderslist,
        ...
        refetch: refetchSupplierOrders } = useGetOrdersQuery({ as, currentPage, rowsPerPage, searchText, orderColumn, sortOrder });
    ...
    const { data: orderList, errors: errorsloadingorders } = useGetOrdersQuery({ as, currentPage, rowsPerPage, searchText, orderColumn, sortOrder });
    const columns = [
        {
            name: 'Customer Name',
