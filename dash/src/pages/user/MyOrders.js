...
const ProductsList = ({ order }) => {
    ...
    return (
        <>
                        ...
                        <tbody>
                            {
                                (order.order_products.length > 0) ?
                                    order.order_products.map((prod, index) => {
                                        ...
                                        return (<tr><td>{prod?.product?.product_name}</td><td>{formatNumberAddCommas(prod?.total_product_cost / prod?.quantity_bought)}</td><td>{prod?.quantity_bought}</td><td>{formatNumberAddCommas(prod?.total_product_cost)}</td>
                                            <td>
            ...
            {/* ///modal add supplier */}
            < Button color="primary" onClick={() => showOrderProductsModal(order)} > View Order Products</Button >
        </>
    );
};
