  ...
  return (
    <ul className="dtr-details p-2 border-bottom ms-1">
      <li className="d-block d-sm-none mb-10">
        <span className="dtr-title">Order Quantity</span>{" "}
        <span className="dtr-data">
          <CartOperations product={data} />
        </span>
      </li>
      <li className="d-block d-sm-none  mb-10">
    ...
    <div style={{ width: "100%" }}>
      ...
      >
        <div>
          <p>
            {" "}
            <Icon name="alert-circle" /> No Items In the Cart.
          </p>
        </div>
        <div>
  ...
  return (
    <span style={{ margin: "10px", display: "flex", flexDirection: "column" }}>
      ...
      <img
        ...
        src={`${process.env.REACT_APP_API_IMAGE_BASE_URL}/${product?.default_image?.image_url}`}
        alt="product"
        className="thumb"
      />
    </span>
...
const CartOperations = ({ product }) => {
  ...
  return (
    <div className="cart-action-container">
      <Button color="light" outline onClick={() => decreaseCounter(product)}>
      ...
      />
      <Button color="light" outline onClick={() => increaseCounter(product)}>
        <Icon name="plus"></Icon>
      </Button>
    </div>
  );
            ...
            {cartItems.length > 0 ? (
              <table className="table table-bordered" style={{ width: "300px" }}>
                <tbody>
                  <tr>
                    <td>
                      <strong>SubTotal (RA)</strong>
                    </td>
                    <td>{formatNumberAddCommas(cart_total)}</td>
                  </tr>
                  <tr>
                    <td>
                      <strong>Taxes (RA)</strong>
                    </td>
                    <td>{formatNumberAddCommas(0.0)}</td>
                  </tr>
                  <tr>
                    <td>
                      <strong>Total Due (RA)</strong>
                    </td>
                    <td>{formatNumberAddCommas(cart_total)}</td>
                  </tr>
                  <tr>
