...
class AdminController extends Controller
{
    ...
    public function progressToNextTier($contribution, $action = 1)
    {
        ...
        if ($assignment_) {
            ...
            return $no_of_completed_paybacks;
            //get contribution tier details
            $contribution_tier = MatrixOption::where('id', $contribution->tier_id)->first();
            $reposnse = $reposnse . 'Completed Paybacks :' . $no_of_completed_paybacks . ' minimumprogression :' . $contribution_tier->minimum_progression_amount . ' Paybackpaybackcount :' . $contribution_tier->payback_count . ' action: ' . $action;
            ...
            if ($backpaiduser['count'] > 0) {
                $backpaidcontribution = Contribution::where('id', $backpaiduser['backpaiduser']->contribution_id_reciving_payback)->first();
                $haspaid = $this->checkIfUserHasPaidForTierandItsApproved($contribution_tier->id, $backpaiduser['backpaiduser']->payee_user_id);
                if ($haspaid == 1) {
                    $reposnse = $reposnse . $this->progressBackPiadUserToNextTier($backpaidcontribution, 1);

                    // $reposnse = $reposnse . 'Payment status' . $haspaid . 'Backpaiduser -' . $this->progressBackPiadUserToNextTier($backpaidcontribution, 1);
                } else {
    ...
    public function progressBackPiadUserToNextTier($contribution, $action = 1)
    {
        ...
        $contribution_tier = MatrixOption::where('id', $contribution->tier_id)->first();
        $contributiontoupp = Contribution::findOrFail($contribution->id);
        $reposnse = 'Contri BPU' . json_encode($contribution) . 'Completed Paybacks BPU' . $no_of_completed_paybacks . 'minimumprogression BPU' . $contribution_tier->minimum_progression_amount . 'Paybackpaybackcount BPU' . $contribution_tier->payback_count . 'action BPU' . $action;
    ...
    public function generateUserOneTimeInviteForSponsoredUsers(Request $request)
    {
        $allusers = User::all();
        foreach ($allusers as $user) {
            $objecttosave['user_id'] = $user->id;
